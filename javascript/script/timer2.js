...
//목표1. 공지1->2->3->4
//1. 어느방향으로 이동하는지, 그 방향에 따른 가로, 세로 크기는 무엇인지? check!!
//아래->위로 이동한다 == top, translateY, 크기 == 세로크기
//좌->우로 이동한다 == left, translateX, 크기 == 가로크기
//2. 가로 또는 세로크기가 결정됐다면? 이동 요소 개수가 몇개인지? 인덱스확인!
...
const itemsHeight = newsItems[0].offsetWidth; //
let currentIndex = 0;
console.log(newsItems, newsList, itemsHeight, currentIndex)

const newsTimer = setInterval(()=>{
    currentIndex++;
    newsList.style.transform = `translateX(-${itemsHeight*currentIndex}px)`;
